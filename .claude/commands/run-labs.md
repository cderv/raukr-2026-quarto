...
3. **Fan out the student(s).** Launch one **`student-participant`** agent per lab (in parallel — one
   ...
   `.claude/archive/reviews/review-YYYY-MM-DD-labrun-<lab>.md`. The agent's own definition carries the
...
7. **Archive.** Friction reports are dated snapshots in `.claude/archive/reviews/`
   (`review-YYYY-MM-DD-labrun-<lab>.md`) — same immutability as the review panel. Record notable
   findings in `.claude/worklog.md`.
