...
5. **Triage.** Read the friction reports + diffs and produce a consolidated triage: the real,
   ...
   **Do not auto-fix** — wait for go-ahead unless told otherwise.
   ...
   Treat every `answer-given` tag as a finding. It means the open page stated the exact answer before
   the participant could attempt the step. Also review any long sequence of `worked-fine` entries
   where no Hint or Solution was opened. Run `just lab-shape-check` to verify the required
   checkpoints and collapsed guidance.
