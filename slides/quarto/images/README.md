...
| File | Used by | Status |
|------|---------|--------|
| `positron-quarto-preview.png` | "Quarto in Positron" slide (`#positron`) | ✅ **in use** — placeholder from the [Quarto docs](https://quarto.org/docs/tools/positron/) (`positron-render.png`); source + preview + Source/Visual toggle. Python example — swap for a local R/penguins capture when available |
| `positron-quarto-r-panes.png` | optional 2nd Positron frame | ⏳ optional |

**Capture note.** Positron is a desktop Electron app that can't run in the web sandbox, so an R/penguins capture is a **local** task. Until
