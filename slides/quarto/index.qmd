...
format:
  revealjs:
    ...
    echo: true
    slide-number: true
    # Preview links in an overlay during fullscreen presentations.
    preview-links: auto
filters:
  - typst-render
typst-render:
  # Store compiled SVGs in a published folder so deployed slides can load them.
...
::: notes
- **Say:** "First, watch the demonstration. Then build it in the lab."
- **Say:** solutions are folded into the lab, so they can self-check.
...
Every check should read `[ok]`, and the last line should be
`>> All good. You are ready for Day 1.`

If any line reads `[FAIL]`, raise your hand.

Details on the [Setup page](../../setup.qmd).
...
## Lab starter checkpoint {#following-along}
...
If Markdown is new to you, see [Markdown Basics](https://quarto.org/docs/authoring/markdown-basics.html).

Quarto adds research-writing features: **figures** and **tables** with
captions, **cross-references** (`@fig-bill`, `@tbl-summary`, `@eq-ratio`), **math**, **inline code**
that reports numbers from the data, and **callouts**.

::: notes
- **Say:** "Markdown handles the prose. Quarto adds the research-writing features."
...
with `fig-` / `tbl-` / `eq-` to be cross-referenceable. `#| fig-alt` is the figure's alt text (the description screen readers announce). Add it to every figure. 

`#| output-location: column` is **slide-only**: a revealjs placement, not article layout.

::: notes
- **Do:** show the rendered figure number, then `@fig-bill` resolving to it.
...
::: notes
- **Do:** show that `tbl-` behaves exactly like `fig-`, with its own counter.
- **Say:** "Same mechanism as figures."
- **Timing:** keep Tables. Cut Math first if needed.
:::
...
This renders as @eq-ratio below:
...
- Inline code is not evaluated in document metadata such as `title`, or in cell options such as
  `#| fig-cap`.
- Inline results are escaped as text. In R, wrap a value in `I()` to preserve its Markdown
  formatting.
...
- **Watch for:** a cell-option caption shows inline-code syntax as literal text instead of
  evaluating it.
:::
...
- **Color-blind-safe colors**: choose a suitable palette. [Okabe-Ito](https://jfly.uni-koeln.de/color/)
  is a common choice for discrete scales in science
...
- **Check contrast**: WCAG AA requires a contrast ratio of at least 4.5:1 for normal text. Quarto's
  built-in `axe` runs axe-core on the
  rendered page:
...
`execute` options work across languages. For R-specific `knitr` options, put the equivalent settings
under `knitr:` in the YAML header:
...
`quarto render` also accepts **`-P name:value`**. Use it to override a document *parameter* and
produce one report per sample or species from a single source. *(Optional bonus in today's lab.)*

::: notes
- **Do:** run `quarto preview`. Edit the document and show the reload. Contrast with `quarto render`.
- **Say:** "Use any editor: RStudio, Positron, or VS Code."
- **Optional:** mention `-P`. The lab bonus uses it.
- **Timing:** keep this brief. Cut parameters first.
:::
...
- **Watch for:** a wrong label prefix.
- **Watch for:** duplicate labels.
- **Watch for:** `#|` options that are not at the top of a cell.
- **Watch for:** the setup cell belongs in the document, not the console.
- **Catch-up:** no `day1-intro/`? Use the Setup page and pair them with a neighbor.
:::
...
**CSL** = *Citation Style Language*. Change `csl:` to another CSL file to apply that journal's
citation style.
:::
::::
...
::: notes
- **Say:** "Type the entry key in your citation. The CSL file formats the citation and reference-list
  entry."
- **Ask:** "Who already manages references with Zotero?"
- **If asked:** CSL means Citation Style Language.
- **Watch for:** a key that does not match an entry renders as `[?]`.
:::

## Where the reference list goes {#refs-placement}
...
::: notes
- **Say:** "One div, two processors. Only citeproc reads it."
...
**Learn more:** [Authors & Affiliations](https://quarto.org/docs/authoring/front-matter.html#authors-and-affiliations), [Title blocks](https://quarto.org/docs/authoring/title-blocks.html)
...
**Typst** (<https://typst.app/>) is a modern typesetting system that is **bundled with Quarto**: nothing to install, no
LaTeX toolchain.
...
To get the PDF *without* touching
...
One `_brand.yml` carries your palette + fonts to a **Typst PDF**, and to the
...
::: notes
- **Do:** show the PDF before and after `_brand.yml`, and point out the color and type change.
- **Say:** the file is already included in `day1-intro/`. Quarto reads it next to the document.
- **Say:** "The same palette can reach plots and tables. Tomorrow it brands a whole project."
- **Watch for:** plots need `theme_brand_*`.
- **Watch for:** the first font render needs network access.
:::
...
- **Watch for:** an incorrect citation key.
- **Watch for:** the first font download.
- **Watch for:** the PDF being written beside its source.
- **Watch for:** a `my-report.qmd` created outside `day1-intro/` (bibliography and brand not found).
:::
