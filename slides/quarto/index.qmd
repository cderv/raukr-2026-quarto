...
- The engine name sits in the braces, so `{{python}}` and `{{julia}}` work the same way.
- Keep the expression simple: one value, computed in a cell.
- It does not run inside YAML (a title, a `#| fig-cap`). Cell options take `!expr` instead.
- The value arrives as text, so Markdown in it is escaped. Wrap it in `I()` to keep the formatting.
...
- **Watch for:** inline code in a caption prints the literal text, it does not compute.
:::
