...
## Quick pause - Following along {#following-along}

The lab's starter (`day1-intro/authoring-starter.qmd`) opens with this cell. Everything on the next
slides assumes it:
...
You probably already know Markdown. If not, let's look at [Markdown Basics](https://quarto.org/docs/authoring/markdown-basics.html).

Quarto adds some research-writing pieces: **figures** and **tables** with
captions, **cross-references** (`@fig-bill`, `@tbl-summary`, `@eq-ratio`), **math**, **inline code**
that reports numbers from the data, and **callouts**. And more.

::: notes
- **Say:** "Do you know Markdown ? And here is what Quarto adds"
...
`execute` config is cross-language. For R specific, R Markdown's configuration and `knitr::opts_chunk$set()` move into the header under `knitr`
...
The same `quarto render` also takes **`-P name:value`**: override a document *parameter* to get one report per sample or species from a single source using Rmarkdown parametrized report. *(Optional bonus in today's lab.)*
...
The header already says which format, so nothing else is needed. 
...
- **Say:** the file already ships in `day1-intro/`. Quarto reads it next to the document.
