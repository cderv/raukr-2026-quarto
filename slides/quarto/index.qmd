...
- **Catch-up:** no exercises folder? Setup page on the site, one `use_course()` line, ~2 min.
:::
...
`citations-starter.qmd` in the same folder (the finished version of this document) and keep going.
:::
...
- **Catch-up:** no `day1-intro/`? Setup page, one `use_course()` line, ~2 min; pair with a
  neighbor meanwhile.
...
The live build is done. The next few slides are concept, no need to type along. Your document is
...
then render it to HTML. We break at **15:00** and resume at **15:30**.
:::
...
You have a clean HTML document. The lab includes a **Part-2 starter** if you need one. To make it a
...
::: {.callout-note title="Follow along"}
Back in your editor: add citations to your Part-1 document (or to `citations-starter.qmd`).
:::
...
your own Part-1 file (or the completed `citations-starter.qmd`), then render it as a **branded Typst PDF**. You'll
